...
// ===== Your Working Details (do not change) =====
...
// Simple session to store user language
...
async function sendLanguageButtons(to) {
    ...
    try {
        ...
        console.log("sendLanguageButtons error:", err.response?.data || err.message);
    }
}

// Send main services list
async function sendMainMenu(to, lang = "english") {
    const text = lang === "marathi" ? "कृपया सेवा निवडा" : "Please choose a service";
    ...
    try {
        ...
        console.log("sendMainMenu error:", err.response?.data || err.message);
    }
}
...
app.post("/webhook", async (req, res) => {
    try {
        ...
        if (type === "text") {
            ...
            return res.sendText(from, "Type 'menu' to open services.");
        }
